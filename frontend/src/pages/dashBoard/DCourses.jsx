...
function Courses() {
  ...
  const openEditCourseModal = (course) => {
    setCourseModal({ isOpen: true, mode: "edit", courseId: course.id });
  };
  ...
  const handleDeleteCourse = async (course) => {
    return await adminService.deleteCourse(course.id);
  };
  ...
  return (
    <div className="max-w-7xl mx-auto">
          ...
          <div className="p-8">
            {loading ? (
              ...
              <div className="grid gap-4">
                {courses.map((course) => (
                  <div key={course.id} className="group bg-white border border-gray-200 rounded-xl hover:shadow-lg hover:border-blue-200 transition-all duration-300 overflow-hidden" >
                    <div className="p-6 flex items-start justify-between">
                      ...
                      <div className="flex items-center gap-2 ml-6">
                        <button onClick={() => addQuestions(course.id)} className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white text-sm font-medium rounded-lg transform hover:scale-105 transition-all duration-200 shadow-md hover:shadow-lg" >
      ...
      <DeleteModal
        ...
        itemDisplayName={deleteModal.course?.title}
      />
    </div>
  );
}
